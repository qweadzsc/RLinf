...
from rlinf.algorithms.registry import get_toolresp_encoder
...
class Searchr1AgentLoopWorker(MultiAgentLoopWorker):
    ...
    def __init__(
        ...
        toolresp_encoder_name = cfg.agentloop.get("toolresp_encoder")
        assert toolresp_encoder_name is not None, (
            "toolresp_encoder must be set in searchr1"
        )
        self.toolresp_encoder = get_toolresp_encoder(toolresp_encoder_name)
    ...
    async def generate_tool_response(
        ...
        tool_response_ids = self.toolresp_encoder(self.tokenizer, tool_messages)
