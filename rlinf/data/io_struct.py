    ...
    def to_actor_batch(
        ...
        if self.rewards is not None and self.rewards.numel() == 0:
            ...
            if self.prev_logprobs is not None:
                batch["prev_logprobs"] = torch.zeros(0, dtype=torch.float32, device=target_device)
            ...
            if self.recompute_prev_logprobs is not None:
                batch["recompute_prev_logprobs"] = torch.zeros(0, dtype=torch.float32, device=target_device)
        ...
        if self.values is not None:
            batch["values"] = self.values.to(target_device)

        if self.returns is not None:
            batch["returns"] = self.returns.to(target_device)
...
class DynamicRolloutResult:
    ...
    def to_actor_batch(
        ...
        if self.prev_logprobs is not None:
            batch["prev_logprobs"] = self.prev_logprobs.to(target_device)

        if self.ref_logprobs is not None:
            batch["ref_logprobs"] = self.ref_logprobs.to(target_device)

        if self.recompute_prev_logprobs is not None:
            batch["recompute_prev_logprobs"] = self.recompute_prev_logprobs.to(target_device)
        ...
        if self.rollout_logprobs is not None:
            ...
            batch["prev_logprobs"] = prev_logprobs.to(target_device)
