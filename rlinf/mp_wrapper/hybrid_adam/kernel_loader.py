...
class _Extension(ABC):
    ...
    def get_jit_extension_folder_path(name):
        ...
        extension_directory = f".cache/torch_extensions/torch{torch_version_major}.{torch_version_minor}_{device_name}-{device_version}-{hash_suffix}"
...
class _CppExtension(_Extension):
    ...
    def build_jit(self) -> None:
        ...
        # load the kernel
        if compiled_before:
            print_rank_0(f"[extension] Loading the JIT-built {self.name} kernel during runtime now")
        else:
            print_rank_0(f"[extension] Compiling the JIT {self.name} kernel during runtime now")
        ...
        if compiled_before:
            print_rank_0(f"[extension] Time taken to load {self.name} op: {build_duration} seconds")
        else:
            print_rank_0(f"[extension] Time taken to compile {self.name} op: {build_duration} seconds")
...
class CpuAdamX86Extension(_CudaExtension):
    ...
    def sources_files(self):
        ret = [
            self.csrc_abs_path("cpu_adam_x86.cpp"),
        ]
        return ret
